...
$$
\begin{aligned}
\min \quad & \sum_{k=1}^{K} Y_k \\[10pt]
\text{s.t.} \quad & \sum_{k=1}^{K} C_{jk} Y_k - \sum_{p: A_{jp}=1} X_p \geq 0, \quad j = 1, \ldots, J \quad \text{(条带平衡)} \\[8pt]
& \sum_{p=1}^{P} B_{ip} X_p \geq d_i, \quad i = 1, \ldots, N \quad \text{(需求满足)} \\[8pt]
& Y_k \geq 0, \quad k = 1, \ldots, K \\[4pt]
...
$$
\begin{aligned}
\max \quad & \sum_{i=1}^{N} d_i \pi_i \\[10pt]
\text{s.t.} \quad & \sum_{j=1}^{J} C_{jk} v_j \leq 1, \quad k = 1, \ldots, K \quad \text{(Y列约束)} \\[8pt]
& -v_j + \sum_{i=1}^{N} B_{ip} \pi_i \leq 0, \quad p \in \mathcal{P}_j \quad \text{(X列约束)} \\[8pt]
& v_j \geq 0, \quad j = 1, \ldots, J \\[4pt]
...
$$
\begin{aligned}
\max \quad & \sum_{j=1}^{J} v_j G_j \\[10pt]
\text{s.t.} \quad & \sum_{j=1}^{J} w_j G_j \leq W \\[8pt]
...
$$
\begin{aligned}
\max \quad & \sum_{i: w_i \leq w_j} \pi_i D_i \\[10pt]
\text{s.t.} \quad & \sum_{i: w_i \leq w_j} l_i D_i \leq L \\[8pt]
...
$$
\begin{aligned}
\max \quad & \sum_{(i,j) \in \mathcal{A}} v_{s(i,j)} \cdot a_{ij} \\[10pt]
\text{s.t.} \quad & \sum_{(0,j) \in \mathcal{A}} a_{0j} = 1 & \text{(起点约束)} \\[8pt]
& \sum_{(i,W) \in \mathcal{A}} a_{iW} = 1 & \text{(终点约束)} \\[8pt]
& \sum_{(i,n) \in \mathcal{A}} a_{in} = \sum_{(n,j) \in \mathcal{A}} a_{nj}, \quad \forall n \in \mathcal{V} \setminus \{0, W\} & \text{(流量守恒)} \\[8pt]
& \sum_{(i,j) \in \mathcal{A}} (j - i) \cdot a_{ij} \leq W & \text{(容量约束)} \\[8pt]
...
$$
\begin{aligned}
\max \quad & \sum_{(i,k) \in \mathcal{A}_j} \pi_{t(i,k)} \cdot a_{ik} \\[10pt]
\text{s.t.} \quad & \sum_{(0,k) \in \mathcal{A}_j} a_{0k} = 1 & \text{(起点约束)} \\[8pt]
& \sum_{(i,L) \in \mathcal{A}_j} a_{iL} = 1 & \text{(终点约束)} \\[8pt]
& \sum_{(i,n) \in \mathcal{A}_j} a_{in} = \sum_{(n,k) \in \mathcal{A}_j} a_{nk}, \quad \forall n \in \mathcal{V}_j \setminus \{0, L\} & \text{(流量守恒)} \\[8pt]
...
$$
\mathcal{S}(\mathbf{G}) = \left\{ \left( \sum_{s=1}^{j-1} \sum_{t=1}^{G_s} w_s + (k-1) w_j, \, \sum_{s=1}^{j-1} \sum_{t=1}^{G_s} w_s + k \cdot w_j \right) : j = 1, \ldots, J, \, k = 1, \ldots, G_j \right\}
$$
...
$$
\begin{cases}
\displaystyle \max_{\mathbf{G}} \sum_{j=1}^{J} v_j G_j \leq 1 & \text{(SP1收敛)} \\[12pt]
...
$$
\begin{cases}
\text{左分支}: & f_{ij} \leq \lfloor f_{ij} \rfloor \\[6pt]
...
$$
(i^*, j^*) = \arg\max_{(i,j): 0 < \{f_{ij}\} < 1} \left( 0.5 - \left| \{f_{ij}\} - 0.5 \right| \right)
$$

其中 $\{f_{ij}\} = f_{ij} - \lfloor f_{ij} \rfloor$ 表示小数部分。
...
$$
f(w) = \max_{j: w_j \leq w} \left\{ f(w - w_j) + v_j \right\}, \quad w = 1, \ldots, W
$$
...
$$
g(l) = \max_{i: l_i \leq l, w_i \leq w_j} \left\{ g(l - l_i) + \pi_i \right\}, \quad l = 1, \ldots, L
$$
