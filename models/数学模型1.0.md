...
| 符号 | 含义 |
|:----:|------|
| $\mathcal{I}$ | 子板类型集合，索引 $i$ |
| $\mathcal{J}$ | 条带类型集合，索引 $j$ |
| $W$ | 母板宽度 |
| $L$ | 母板长度 |
| $w_i$ | 子板 $i$ 的宽度 |
| $l_i$ | 子板 $i$ 的长度 |
...
若存在多个子板宽度相同，则对应同一条带类型。记条带类型 $j$ 的宽度为 $w_j$。
...
$$C_{jq} \in \mathbb{Z}_+ : \text{模式 } q \text{ 在一张母板上产生的 } j \text{ 类条带数量}$$

- **Strip 模式 (条带模式)**：$p \in \mathcal{P}$

$$B_{ip} \in \mathbb{Z}_+ : \text{模式 } p \text{ 在一条条带上产生的 } i \text{ 类子板数量}$$
...
$$A_{j(p),p} = 1, \quad A_{jp} = 0 \quad (\forall j \ne j(p))$$
...
| 变量 | 含义 |
|:----:|------|
| $Y_q \in \mathbb{Z}_+$ | 使用 stock 模式 $q$ 的次数 (母板用量) |
| $X_p \in \mathbb{Z}_+$ | 使用 strip 模式 $p$ 的次数 (条带消耗次数) |
...
$$\sum_{p \in \mathcal{P}} B_{ip} X_p \ge d_i, \quad \forall i \in \mathcal{I}$$

**(S) 条带平衡 (允许多余条带)**

$$\sum_{q \in \mathcal{Q}} C_{jq} Y_q - \sum_{p \in \mathcal{P}} A_{jp} X_p \ge 0, \quad \forall j \in \mathcal{J}$$

$$X_p \in \mathbb{Z}_+, \quad Y_q \in \mathbb{Z}_+$$

### 4.3 受限主问题 (RMP) 与 LP 松弛

列生成中对有限列集合求解 LP 松弛：

$$X_p \ge 0, \quad Y_q \ge 0$$
...
| 对偶变量 | 对应约束 |
|:--------:|----------|
| $\pi_i \ge 0$ | 需求约束 (D)，$\forall i \in \mathcal{I}$ |
| $v_j \ge 0$ | 条带平衡约束 (S)，$\forall j \in \mathcal{J}$ |

对任意候选列，采用"利润式"入列判据 (将经典 reduced cost 取负号)：

**条带列 (Strip 列) $p$ 的利润：**

$$RC_X(p) = \sum_{i \in \mathcal{I}} \pi_i B_{ip} - \sum_{j \in \mathcal{J}} v_j A_{jp} = \sum_{i} \pi_i B_{ip} - v_{j(p)}$$
...
$$\sum_{i \in \mathcal{I}(j)} l_i D_i \le L, \quad D_i \in \mathbb{Z}_+$$
...
$$\sum_{j \in \mathcal{J}} w_j G_j \le W, \quad G_j \in \mathbb{Z}_+$$
...
若用"0 到终点的单位流路径"刻画装载，但网络中没有 slack (空走/损耗) 结构，则会将 $\le$ 背包误收紧为 exact fill。本文采用 **loss arcs**，保证 Arc-Flow 与背包可行域严格等价。

### 7.2 通用 Arc-Flow 模板 (loss arcs)

给定容量 $C \in \mathbb{Z}_+$，构造 DAG：

- **节点**：$V = \{0, 1, \ldots, C\}$
- **物品弧**：$A^{\text{item}} = \{(t, t+s) : s \in \mathcal{S}, t+s \le C\}$
- **损耗弧 (slack)**：$A^{\text{loss}} = \{(t, t+1) : t = 0, \ldots, C-1\}$
...
$$\sum_{a \in \delta^+(0)} x_a = 1, \quad \sum_{a \in \delta^-(C)} x_a = 1$$

$$\sum_{a \in \delta^-(n)} x_a = \sum_{a \in \delta^+(n)} x_a, \quad \forall n = 1, \ldots, C-1$$
...
上述 Arc-Flow 与整数背包 $\sum s_k z_k \le C, z_k \in \mathbb{Z}_+$ 在可实现装载集合上等价：未用容量由损耗弧表示。
...
$$D_i = \sum_{t=0}^{L-l_i} x_{t, t+l_i}, \quad i \in \mathcal{I}(j)$$
...
$$\delta^X_{a,p} = \begin{cases} 1, & \text{列 } p \text{ 的 SP2 路径使用弧 } a \\ 0, & \text{否则} \end{cases}$$
...
- 左子节点：$F^X_a \le \lfloor F^{X*}_a \rfloor$
- 右子节点：$F^X_a \ge \lceil F^{X*}_a \rceil$
...
$$\mathcal{A}^{\text{cand}} = \{a : \text{frac}(F^{X*}_a) \in (\varepsilon_{\text{int}}, 1 - \varepsilon_{\text{int}})\}$$

选择规则为按序最小化/最大化：
1. $|\text{frac}(F^{X*}_a) - 0.5|$ 最小
2. $|F^{X*}_a|$ 最大
...
$$\delta^Y_{a,q} \in \{0, 1\}, \quad F^Y_a = \sum_{q \in \mathcal{Q}} \delta^Y_{a,q} Y_q$$
...
$$\sum_{a \in \mathcal{A}_{\mathcal{N}}} \mu_a \delta^X_{a,p}$$
...
| 参数 | 值 | 说明 |
|:----:|:--:|------|
| $\varepsilon_{\text{int}}$ | $10^{-6}$ | 整数判别阈值 |
| $\varepsilon_{\text{rc}}$ | $10^{-7}$ | 入列阈值 (利润式) |

**整数判别**：$\text{integer}(x) \Leftrightarrow |x - \text{round}(x)| \le \varepsilon_{\text{int}}$

**入列条件**：仅当定价最优利润 $RC > \varepsilon_{\text{rc}}$ 才加入列
