...
本文给出一种层级两阶段切割问题的标准化建模与求解模板：每张 stock (宽 $W$、长 $L$) 先沿宽度方向切成若干条**全长 strip**，再在每条 strip 上沿长度方向切成 items。我们采用一个**集成主问题**将第一阶段的 strip 供给与第二阶段的 strip 消耗耦合，并通过**列生成 (Column Generation) + 分支定价 (Branch-and-Price)** 求解。定价子问题为 knapsack，可用带 **loss arcs (显式 slack)** 的 arc-flow 形式实现，以保证与 $\le$-knapsack 的严格等价。我们明确给出 profit-form 与 reduced cost 的一致符号约定，并使用**聚合弧使用量 (aggregated arc usage)** 的分支规则，使分支约束能够通过对偶乘子自然进入定价。最后给出一个无需人工变量、对任意可行实例都能初始化可行的 RMP 构造。
...
1. **profit-form 与标准 reduced cost 的严格等价**：统一对偶变量与利润的定义，使得 $\text{profit} > 0 \Leftrightarrow \text{reduced cost} < 0$ 成为严格等价关系。
...
4. **RMP 初始可行列集合的标准构造**：给出一个无需人工变量、对任意可行实例都可构造可行初解的 $(P_0, Q_0)$ 及可行性证明思路。
...
- Stock 尺寸为 $W \times L$ (width $\times$ length)，$W, L \in \mathbb{Z}_+$。
- Item type 集合为 $I$。每个 $i \in I$ 尺寸为 $(w_i, l_i)$，需求为 $d_i \in \mathbb{Z}_+$。
...
并将每个 $j \in J$ 视为"宽度为 $w_j$"的一类 strip。
...
- 允许超产 (overproduction allowed)：需求约束采用 $\ge$。
...
- **Stock patterns**：集合 $Q$。对 $q \in Q$，

$$C_{jq} \in \mathbb{Z}_+, \quad \forall j \in J$$

表示 pattern $q$ 在一张 stock 上切出多少条 type-$j$ strip。

- **Strip patterns**：集合 $P$。对 $p \in P$，

$$B_{ip} \in \mathbb{Z}_+, \quad \forall i \in I$$

表示 pattern $p$ 在一条 strip 上生产多少个 type-$i$ item。

- **Strip-type indicator**：对 $p \in P$，

$$A_{jp} \in \{0, 1\}, \quad \forall j \in J$$

且每个 strip pattern **恰好消耗一种 strip type**：存在唯一 $j(p) \in J$ 使 $A_{j(p),p} = 1$，其余为 0。

### 2.2 主问题变量

$$Y_q \in \mathbb{Z}_+, \quad q \in Q : \text{使用 stock pattern } q \text{ 的次数}$$

$$X_p \in \mathbb{Z}_+, \quad p \in P : \text{使用 strip pattern } p \text{ 的次数}$$
...
$$X_p \ge 0, \quad Y_q \ge 0, \quad X_p, Y_q \in \mathbb{Z}_+$$
...
- $\pi_i \ge 0$ 对应 item 需求约束：

$$\sum_{p} B_{ip} X_p \ge d_i$$

- $v_j \ge 0$ 对应 strip 平衡约束：
...
$$\text{profit}_X(p) := \sum_{i \in I} \pi_i B_{ip} - \sum_{j \in J} v_j A_{jp} = \sum_{i \in I} \pi_i B_{ip} - v_{j(p)}$$
...
$$\text{rc}_X(p) = -\text{profit}_X(p), \quad \text{rc}_Y(q) = -\text{profit}_Y(q)$$
...
给定容差 $\varepsilon_{rc} > 0$：若定价最优 profit $> \varepsilon_{rc}$，则加入相应列。
...
$$\sum_{i \in I(j)} l_i D_i \le L, \quad D_i \in \mathbb{Z}_+$$

若最优值 $> \varepsilon_{rc}$，生成新 strip 列 $p$，满足 $j(p) = j$ 且 $B_{ip} = D_i$。
...
$$\sum_{j \in J} w_j G_j \le W, \quad G_j \in \mathbb{Z}_+$$

若最优值 $> \varepsilon_{rc}$，生成新 stock 列 $q$，满足 $C_{jq} = G_j$。
...
给定容量 $C \in \mathbb{Z}_+$，节点集 $V = \{0, 1, \ldots, C\}$。定义 loss arcs

$$A^{loss} := \{(t, t+1) : t = 0, \ldots, C-1\}$$

用于显式表示 slack，从而与 $\le$-knapsack 严格等价 (避免被误建成"必须恰好填满")。
...
对每条弧 $a = (t, t+l_i, i)$ 设二元变量 $x_a \in \{0, 1\}$ 表示路径是否使用该弧 (允许平行弧：同起点同终点但标签不同)。
...
SP1 可在 $V = \{0, \ldots, W\}$ 上定义 item arcs $(t, t+w_j)$ 与 loss arcs $(t, t+1)$，并用 reward $v_j$ 构成 knapsack 定价。
...
$$\delta^X_{a,p} = \begin{cases} 1, & \text{若 strip 列 } p \text{ 的 SP2 路径使用弧 } a \\ 0, & \text{否则} \end{cases}$$
...
$$F^X_a \le \lfloor F_a^{X*} \rfloor \quad \text{vs.} \quad F^X_a \ge \lceil F_a^{X*} \rceil$$

### 6.3 分支约束如何进入定价 (dual-adjusted rewards)

每条继承下来的聚合弧使用量约束在节点 LP 上产生对偶乘子 $\mu_a$。定价时可等价地通过调整对应弧的 reward 吸收该约束：
...
- $\forall i \in I : l_i \le L$ (否则 item 长度放不进任何 strip)
- $\forall j \in J : w_j \le W$ (否则该 strip 宽度放不进 stock)

### 7.2 初始化列集合 $(P_0, Q_0)$

**(A) 初始 stock 列：对每个 $j \in J$**

构造 stock pattern $q^{(j)}$，只切出 1 条 type-$j$ strip，其余宽度作为 waste：

$$C_{j, q^{(j)}} = 1, \quad C_{j', q^{(j)}} = 0 \quad (j' \neq j)$$

令

$$Q_0 := \{q^{(j)} : j \in J\}$$

**(B) 初始 strip 列：对每个 $i \in I$**

构造 strip pattern $p^{(i)}$，消耗 strip type $j = w_i$，只生产 1 个 item $i$，其余长度作为 waste：

$$j(p^{(i)}) = w_i, \quad B_{i, p^{(i)}} = 1, \quad B_{i', p^{(i)}} = 0 \quad (i' \neq i)$$
...
- 对每个 $i$，需求约束满足：$\sum_p B_{ip} X_p \ge d_i$
- 对每个 $j$，strip 平衡满足 (可取等号)：供给条数等于消耗条数

因此 RMP 在 $(P_0, Q_0)$ 下可行。

---

## 8. 算法流程 (Root CG + B&P)

1. **初始化**：构造 $(P_0, Q_0)$ 并解 RMP 的 LP 松弛。
2. **定价循环**：读取对偶 $(\pi, v)$，对所有 $j$ 解 SP2，并解一次 SP1；加入所有 profit $> \varepsilon_{rc}$ 的列，重新优化；直至无改进列。
